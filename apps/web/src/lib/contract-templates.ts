...
const COMMON_LIABILITY = {
  ...
  services_provider:
    "Оплата после подписания акта. Пени за просрочку оплаты 0,1% в день (не более 10% цены). Ответственность исполнителя ограничена суммой фактически полученной оплаты, за исключением умысла.",
  services_customer:
    "Оплата по итогам приёмки. Право отказаться от приёмки при существенных недостатках. Пени за просрочку оказания услуг 0,1% в день. Право привлечь третьих лиц за счёт исполнителя при срыве сроков.",
};
...
export const CONTRACT_TYPES: ContractTypeDef[] = [
  ...
  },
  {
    ...
    titleDefault: "Договор оказания услуг",
    positions: [
      {
        id: "supplier",
        label: "Мы — поставщик (исполнитель)",
        hint: "Защита исполнителя: предмет, приёмка, оплата, лимит ответственности",
        defaults: {
          parties: "Исполнитель (поставщик услуг): [наша компания], ИНН…\nЗаказчик: [контрагент], ИНН…",
          subject: "Оказание услуг: [описание услуг / результат / объём], критерии качества",
          payment_terms: "Аванс 30%, остаток в течение 10 рабочих дней после подписания акта сдачи-приёмки",
          timeline_terms: "Срок оказания услуг — [N] рабочих дней с даты аванса / с даты подписания договора",
          acceptance_terms:
            "Приёмка по акту. Мотивированный отказ — в течение 5 рабочих дней; при молчании услуги считаются принятыми",
          warranty_terms: "Устранение недостатков услуг — в разумный срок по мотивированной претензии (не более 15 рабочих дней)",
          liability_terms: COMMON_LIABILITY.services_provider,
          special_terms:
            "Право приостановки оказания услуг при просрочке оплаты более 10 дней. Подсудность — по месту нахождения исполнителя. Применимое право — РФ (гл. 39 ГК РФ).",
        },
      },
      {
        id: "customer",
        label: "Мы — заказчик",
        hint: "Контроль результата, сроков, качества и оплаты после приёмки",
        defaults: {
          parties: "Заказчик: [наша компания], ИНН…\nИсполнитель (поставщик услуг): [контрагент], ИНН…",
          subject: "Оказание услуг: [описание], измеримый результат / KPI, отчётность",
          payment_terms: "Оплата в течение 15 календарных дней после подписания акта без замечаний; удержание 5% до устранения недостатков",
          timeline_terms: "Календарный график / этапы; промежуточная отчётность по запросу заказчика",
          acceptance_terms:
            "Приёмка заказчиком; право не подписывать акт при существенных недостатках и потребовать безвозмездного устранения",
          warranty_terms: "Гарантия качества услуг не менее [N] месяцев с даты приёмки",
          liability_terms: COMMON_LIABILITY.services_customer,
          special_terms:
            "Право одностороннего отказа от договора с оплатой фактически оказанных услуг (ст. 782 ГК РФ) — условия согласовать. Подсудность — по месту нахождения заказчика.",
        },
      },
    ],
    fields: [
      { key: "parties", label: "Стороны и реквизиты", kind: "textarea", required: true, placeholder: "Исполнитель / Заказчик, ИНН, представители" },
      { key: "subject", label: "Предмет (услуги / результат)", kind: "textarea", required: true },
      { key: "price", label: "Цена / стоимость услуг", kind: "text", placeholder: "500 000 руб. с НДС" },
      { key: "payment_terms", label: "Условия оплаты", kind: "textarea" },
      { key: "timeline_terms", label: "Сроки оказания", kind: "textarea" },
      { key: "acceptance_terms", label: "Приёмка услуг", kind: "textarea" },
      { key: "warranty_terms", label: "Качество и гарантии", kind: "textarea" },
      { key: "liability_terms", label: "Ответственность", kind: "textarea" },
      { key: "special_terms", label: "Особые условия", kind: "textarea" },
    ],
  },
