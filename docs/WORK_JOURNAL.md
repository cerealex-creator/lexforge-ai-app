...
## 2026-07-18 — Сообщение о недоступной БД для продакшена
**Запрос:** На сервере при генерации договора текст про Docker Desktop / make up.
**Сделано:** Тексты `DB_UNAVAILABLE_MSG` и клиентский fallback заменены на серверные подсказки.
**Файлы:** `apps/api/database.py`, `apps/web/src/lib/api.ts`.
**Статус:** В процессе — на сервере нужно поднять Postgres и при желании задеплоить правку текста.
